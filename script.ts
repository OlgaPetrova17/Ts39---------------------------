class Phone {
    ...
    receiveCall(name: string, person: Phone): void;
    receiveCall(name: string, person?: Phone): void {
        if (person === undefined) {
            console.log(`Звонит ${name} на телефоне ${this.model}. Вес ${this.weight}`)
        }
        else console.log(`Вес телефона  = ${person.weight} граммов. На него звонит ${name} с телефона с номером ${person.number} и моделью телефона ${person.model}`)
    }
}
...
function handlePhone(pm: Phone, r: number): void {
